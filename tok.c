...
TokenTable *results(BPE *bpe, Pool *pool) {
    ...
    table->merges = palloc(pool, sizeof(uint16_t) * 2 * (table->count - 256));
    ...
    for (int i = 0; i < table->count; i++) {
        ...
        if (i < 256) {
            ...
            table->merges[i - 256][0] = info->left;
            table->merges[i - 256][1] = info->right;
        }
...
void tok_train(BPE *bpe, Pool *pool) {
    for (int c = 0; c < 256; c++) tok_define_char(bpe->vocab, c, pool);
